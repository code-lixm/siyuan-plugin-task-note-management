<script lang="ts">
  ...
  function clampYearlyDays() {
    ...
    yearlyStartDay = Math.max(
      1,
      Math.min(maxDayOfMonth(yearlyStartMonth), yearlyStartDay),
    );
    yearlyEndDay = Math.max(
      1,
      Math.min(maxDayOfMonth(yearlyEndMonth), yearlyEndDay),
    );
  }
  ...
  onMount(async () => {
    ...
    Object.keys(groupedProjects).forEach((statusKey) => {
      ...
      const nonArchivedProjects = statusProjects.filter((project) => {
        const projectStatus =
          projectManager.getProjectById(project.id)?.status || "doing";
  ...
  function selectFilter(filter: FilterConfig) {
    ...
    const customRange = filter.dateFilters.find(
      (df) => df.type === "custom_range",
    );
    if (customRange) {
    ...
    const futureXDays = filter.dateFilters.find(
      (df) => df.type === "future_x_days",
    );
    if (futureXDays) {
    ...
    const yearlyRange = filter.dateFilters.find(
      (df) => df.type === "yearly_date_range",
    );
    if (yearlyRange) {
    ...
    <div class="filter-list-content">
      {#each filters as filter (filter.id)}
        ...
        <div
          ...
          class:drag-over-above={dragTargetId === filter.id &&
            dragPosition === "above"}
          class:drag-over-below={dragTargetId === filter.id &&
            dragPosition === "below"}
          draggable={true}
