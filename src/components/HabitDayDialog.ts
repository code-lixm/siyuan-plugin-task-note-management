...
export class HabitDayDialog {
        ...
        if (entries.length === 0) {
            ...
            entries.forEach((entry, idx) => {
                ...
                delBtn.addEventListener('click', () => {
                    confirm(i18n("confirmDeleteEntry"), i18n("confirmDeleteEntryMsg", { title: this.habit.title, date: this.dateStr, index: String(idx + 1) }), async () => {
    ...
    private openAddEntryDialog() {
        ...
        const dialog = new Dialog({ title: i18n("addDayCheckIn"), content: '<div id="habitDayAddEntry"></div>', width: '420px', height: '360px' });
        ...
        const timeLabel = document.createElement('label'); timeLabel.textContent = i18n("timeInputLabel");
        ...
        const emojiLabel = document.createElement('div'); emojiLabel.textContent = i18n("checkInStatusLabel"); emojiLabel.style.cssText = 'font-weight:bold;'; container.appendChild(emojiLabel);
        ...
        const noteLabel = document.createElement('div'); noteLabel.textContent = i18n("noteOptionalLabel"); container.appendChild(noteLabel);
        ...
        const cancelBtn = document.createElement('button'); cancelBtn.className = 'b3-button'; cancelBtn.textContent = i18n("cancel"); cancelBtn.addEventListener('click', () => dialog.destroy());
        const saveBtn = document.createElement('button'); saveBtn.className = 'b3-button b3-button--primary'; saveBtn.textContent = i18n("save");
        saveBtn.addEventListener('click', async () => {
            if (!selectedEmoji) { showMessage(i18n("selectCheckInStatus"), 2000, 'error'); return; }
    ...
    private async openEditEntryDialog(index: number) {
        ...
        const dialog = new Dialog({ title: i18n("editDayCheckIn"), content: '<div id="habitDayEditEntry"></div>', width: '380px', height: '360px' });
