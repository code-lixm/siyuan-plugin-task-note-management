<script lang="ts">
  ...
  const AUDIO_URL_PREFIX =
    "/data/storage/petal/siyuan-plugin-task-daily/audios/";

  /** 获取指定 key 的音频文件列表（合并内置声音并过滤已删除项） */
  function getAudioFilesForKey(key: string): { name: string; path: string }[] {
    const userList: AudioFileItem[] =
      (settings.audioFileLists ?? {})[key] ?? [];
    const defaultList: AudioFileItem[] =
      (DEFAULT_SETTINGS.audioFileLists ?? {})[key] ?? [];
  ...
  async function deleteAudioFileForKey(url: string, key: string) {
    ...
    const currentList: AudioFileItem[] = [
      ...(settings.audioFileLists[key] ?? []),
    ];
    ...
    if (settings.audioSelected && settings.audioSelected[key] === url) {
      ...
      settings.audioSelected[key] =
        remaining.length > 0 ? remaining[0].path : "";
    }
  ...
  async function downloadOnlineAudio(url: string, key: string) {
    ...
    try {
      ...
      const fileName =
        url.split("/").pop()?.split("?")[0] || "online_audio.mp3";
  ...
  export const useShell = async (
    cmd: "showItemInFolder" | "openPath",
    filePath: string,
  ) => {
    try {
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          description:
            i18n("calendarMultiDaysCountDesc") ||
            "设置多天视图显示的天数，默认为3天",
        },
        {
    ...
    },
    {
      ...
      items: [
        ...
        },
        {
          ...
          description:
            i18n("icsDailySyncTimeDesc") ||
            "设置每天几点同步，格式 HH:MM（如 08:00）",
          placeholder: "08:00",
        },
        {
          key: "icsSilentUpload",
  ...
  onMount(() => {
    ...
    const settingsUpdateHandler = async (e: Event) => {
      ...
      if (typeof settings.weekStartDay === "string") {
        ...
        settings.weekStartDay = isNaN(parsed)
  ...
  .audio-row {
    ...
    &--selected {
      background: color-mix(
        in srgb,
        var(--b3-theme-primary) 8%,
        var(--b3-theme-background)
      );
      border-color: color-mix(
        in srgb,
        var(--b3-theme-primary) 30%,
        transparent
      );
    }
